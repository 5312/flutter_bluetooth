...
class _CloudState extends State<Cloud> {
  ...
  void _onPcDeviceFound(Map<String, dynamic> data) {
    debugPrint('发现DrillTrack服务器: ${data.toString()}');
    SmartDialog.showToast('发现服务器: ${data['name'] ?? "DrillTrack Server"}');
  ...
  void _discoverPcDevices() {
    ...
    _initUdpService().then((_) {
      ...
      SmartDialog.showToast('正在搜索DrillTrack服务器...');
      debugPrint('开始搜索DrillTrack服务器');
      ...
      Future.delayed(const Duration(seconds: 15), () {
        if (mounted && _isDiscovering && !_pcConnected) {
          ...
          SmartDialog.showToast('未发现DrillTrack服务器，请确认服务器是否运行，并检查网络连接');
  ...
  // 批量同步选中的项目
  Future<void> _batchSyncToPc() async {
    if (!_pcConnected) {
      SmartDialog.showToast('未连接到服务器，请先搜索服务器');
      return;
    }
    ...
    if (_selectedItems.isEmpty) {
      SmartDialog.showToast('请选择要同步的项目');
      return;
    }
    
    List<RepoModel> selectedRepos = [];
    for (var id in _selectedItems) {
      for (var repo in _repoModels) {
        if (repo.id == id) {
    ...
    if (selectedRepos.isEmpty) {
      SmartDialog.showToast('未找到选中的项目');
      return;
    }
    
    SmartDialog.showLoading(msg: '正在批量同步数据...');
    try {
      Map<int, bool> results = await _pcService.batchSyncData(selectedRepos);
      ...
      SmartDialog.showToast('批量同步错误: ${e.toString()}');
    } finally {
      SmartDialog.dismiss();
    }
  }
    ...
    setState(() {
      ...
      if (_selectAll) {
        ...
        for (var employee in employees) {
          _selectedItems.add(employee.id);
        }
      } else {
        // 取消所有选择
                  ...
                  children: [
                          ...
                          children: [
                            ElevatedButton(
                              style: ElevatedButton.styleFrom(
                                backgroundColor: _selectAll ? Colors.blue : Colors.blueAccent,
                            ...
                            ElevatedButton(
                              ...
                              child: const Text('批量同步', 
                                style: TextStyle(fontSize: 16, color: Colors.blue)),
                              onPressed: _batchSyncToPc,
                            ),
                          ],
                        ),
                        Row(
                    ...
                    if (_pcConnected && _pcService.currentServer != null) ...[
                      ...
                      Row(
                        ...
                        children: [
                          Text(
                            '服务器: ${_pcService.currentServer!.name} (${_pcService.currentServer!.ipAddress}:${_pcService.currentServer!.httpPort})',
